...
SRC_URI = "file://socketcan0-interface.service \
	   ...
	   file://rpmsg-load.service"

#here we specify the source directory, where we can do all the building and expect sources to be placed
S = "${WORKDIR}"

SYSTEMD_SERVICE_${PN} = "rpmsg-load.service socketcan0-interface.service socketcan1-interface.service"
...
do_install() {
	...
	install -m 0644 ${WORKDIR}/rpmsg-load.service ${D}${systemd_unitdir}/system
	...
	ln -sf ${systemd_unitdir}/system/rpmsg-load.service \
         ${D}${sysconfdir}/systemd/system/multi-user.target.wants/rpmsg-load.service
}
